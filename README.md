...
IAW 5AM (HW610)** ECU, running on a **Raspberry Pi-compatible SBC** (Ubuntu 24.04). On power-up the board brings
...
🔧 [Protocol reference](docs/PROTOCOL.md) · 📦 [Install and upgrade](docs/DEPLOY.md)
...
- **Full parameter scan** (Testing tab) — sweep `0x00..0xFF` (addressed / both framings), auto-stop
  timer with a first-pass progress readout; writes `scan-*.ndjson` + a wide `scan-*.csv`. The tool to
  map new rli — heavy enough to drop the session, so it sits with the other diagnostics, not with the
  everyday logging.
...
- **Web UI** tabs:
  - *Logger* — decoded/raw toggles, live values + 3 s sparklines, poll rate, and the channel list
    in three sections: confirmed channels first, then the ones that carry a name but are still to
  ...
  - *Testing* — **read/clear fault codes** (SAE decode, localized descriptions, *stored vs current*
    ...
    if the browser goes away), the **full parameter scan** — it lives here, at the bottom just above
    the reference blocks, because it is a diagnostic tool rather than a recording mode — an **ECU
    profile reference** (65 Guzzi models → ECU → protocol) and the status-bitfield tables. The diagnostic session (`83 03 00 FF 00 FF 00` +
...
| Part | What |
|---|---|
| Board | Raspberry Pi-compatible arm64 SBC, Ubuntu 24.04, kernel 6.6 |
...
Three ways in, one reference: **[docs/DEPLOY.md](docs/DEPLOY.md)** — first install, upgrading from
the dev host, upgrading from the phone, versioning and recovery, in all eight UI languages
([de](docs/DEPLOY.de.md) · [es](docs/DEPLOY.es.md) · [fr](docs/DEPLOY.fr.md) · [it](docs/DEPLOY.it.md) · [nl](docs/DEPLOY.nl.md) · [bg](docs/DEPLOY.bg.md) · [ru](docs/DEPLOY.ru.md)).

`install.sh` is first-time provisioning only. Every change afterwards ships with `./deploy.sh`
**from the dev host**: it runs the offline test suite (`--no-tests` skips it), rsyncs the whole
repo — not just `app/`, since `config/` is read at runtime — restarts the service and then
verifies by checksum in both directions, failing if anything still differs. `BOARD_HOST`,
`BOARD_USER` and `BOARD_PASS` come from `.deploy.env` (see `.deploy.env.example`); with no
password set it uses ssh key auth. The board is not a git checkout — it holds whatever was last
rsynced, so an undeployed change means the bike runs old code.

Away from the dev host, **Config → System** takes an archive instead: `./release.sh` builds
`dist/onboard-logger-<version>-<sha>.tar.gz`, the phone carries it, and the board checks it, runs its
own offline test suite against it and swaps it in — restoring the previous version by itself if the
new one does not come up. Details and the recovery steps are in
...
- Test/adaptation services live in a diagnostic session: `81` → `83 03 00 FF 00 FF 00`
  → `10 81` (session **0x81**, not 0x85), closed with `20` + `82`. There is no separate "test mode".

Full details and the complete parameter/actuator/DTC tables are in
...
./release.sh                                  # dist/onboard-logger-<version>-<sha>.tar.gz
```
...
docs/          PROTOCOL.md (protocol reference) + .ru.md,
               DEPLOY.md (install + upgrade, all 8 UI languages)
systemd/ udev/ install.sh (first-time provisioning), deploy.sh (ship changes from the dev host),
               release.sh (pack an archive the board's own update button accepts)
```
