...
def test_an_update_log_is_a_board_artefact_not_a_ride():
    """_log_kind is the single place that decides what a file is; the Logs tab
    asks for rides and Config -> System for the board's own files."""
    with tempfile.TemporaryDirectory() as tmp:
        root = _root(tmp)
        (root / DAY / "update-20260906-120000.log").write_text("swap\n")
        try:
            assert main._log_kind("update-20260906-120000.log") == "update"
            board = asyncio.run(main.list_logs(kind="board"))
            assert "update-20260906-120000.log" in {f["file"] for f in board["files"]}
            ride = asyncio.run(main.list_logs(kind="ride"))
            assert "update-20260906-120000.log" not in {f["file"] for f in ride["files"]}
        finally:
            _clear()
