...
const FILES = [
  ...
  { name: "01-09-2026/update-20260901-210000.log", file: "update-20260901-210000.log",
    day: "01-09-2026", size: 700, mtime: 1787998000, kind: "update", zip: false },
...
test("the System list groups by day, newest first, and folds the older days", async () => {
  ...
  has(labels[0], "3 " + sb.window.I18N.en["logs.groupCount"]);
});

test("all three board kinds are badged and only the System list carries them", async () => {
  const sb = makeSandbox();
  await fire(sb, "#boardLogsRefresh");
  ...
  has(html, "update-20260901-210000.log");
  ...
  assert(!ride.includes("update-2026"), "an update log must not reach the Logs tab");
});

test("the update filter narrows to the update logs alone", async () => {
  const sb = makeSandbox();
  await fire(sb, "#boardLogsRefresh");
  const btn = { closest: () => btn, dataset: { f: "update" }, classList: { toggle() {} } };
  await fire(sb, "#boardLogFilter", "click", { target: { closest: () => btn } });
  const html = listHtml(sb, "#boardLogsList");
  has(html, "update-20260901-210000.log");
  assert(!html.includes("diag-2026"), "kind=update must hide the diagnostics files");
  assert(!html.includes("fw-reading"), "kind=update must hide the firmware logs");
});
...
test("every row offers a download; only a plain file is also a link to its text", async () => {
  ...
  has(html, "/api/update/log.txt?file=");   // the third kind reads its own endpoint
