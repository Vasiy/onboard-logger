...
// sends none, and there `default` is what the grouping used to key off.
const pgroup = (ch) => ch.group || (ch.default ? "known" : "unknown");

// The rows that carry a checkbox, wherever they sit — the unidentified rli live
// one level down, inside the fold.
const paramRows = () => $$(".prow", $("#params"));

// That fold is collapsed by default and a preset can tick channels inside it, so
// opening it has to be callable from outside the render.
function setUnknownOpen(open) {
  const box = $("#params"), hdr = $(".unknown-toggle", box), wrap = $(".unknown-wrap", box);
  if (!hdr || !wrap) return;
  wrap.hidden = !open;
  hdr.textContent = `${open ? "▾" : "▸"} ${t("params.unknown")} (${$$(".prow", wrap).length})`;
}
...
function renderParams() {
  ...
  const unknown = catalog.filter((c) => pgroup(c) === "unknown");
  known.forEach((ch) => box.appendChild(mkParamRow(ch)));
  if (check.length) {           // named, but the meaning still has to come off the bike
    const lbl = document.createElement("div");
    lbl.className = "params-group";
    lbl.textContent = `${t("params.check")} (${check.length})`;
    lbl.title = t("params.checkHint");
    box.appendChild(lbl);
    check.forEach((ch) => box.appendChild(mkParamRow(ch)));
  }
  if (unknown.length) {                       // collapse the long unidentified-rli list
    ...
    hdr.addEventListener("click", () => setUnknownOpen(wrap.hidden));
    unknown.forEach((ch) => wrap.appendChild(mkParamRow(ch)));
    box.appendChild(hdr);
    box.appendChild(wrap);
    setUnknownOpen(unknown.some((c) => selectedKeys.has(c.key)));   // open if any ticked
  }
...
async function onSelectChange() {
  ...
  // while a preset is open for editing the tick *is* the preset — there is no
  // separate "done", so a lost link cannot swallow the edit
  if (editIdx >= 0 && presets) {
    presets[editIdx].keys = [...selectedKeys];
    await savePresets();
  }
  renderPresets();          // green follows set equality, so it is recomputed here
  renderPresetCost();
}

$("#selAll").addEventListener("click", () => { paramRows().forEach((r) => ($("input", r).checked = true)); onSelectChange(); });
$("#selNone").addEventListener("click", () => { paramRows().forEach((r) => ($("input", r).checked = false)); onSelectChange(); });
...
// keeps only the picked channels and lays them out as big tiles, and the preset
// buttons sit above them — swapping the whole set is the one selection change
// that stays available there. Ticking a single channel is a "pick" job, and so
// are all/none.
...
$$("#paramMode button").forEach((b) =>
  b.addEventListener("click", () => {
    ...
    if (paramMode !== "edit" && editIdx >= 0) { editIdx = -1; setEditLock(false); }
    try { localStorage.setItem("paramMode", paramMode); } catch (e) {}
    applyParamMode();
  }));

// ---------- parameter presets ----------
// Three named channel sets, kept on the board (presets.json). Which one is
// "active" is deliberately never stored: a button lights up when the live
// selection equals its set, so a manual tick puts it out by itself and a reload
// finds the state again with no bookkeeping to go stale. `lastIdx` is memory
// only — it marks the preset the rider came from, so a gone-grey button still
// says which one was departed from.
const PRESET_NAME_MAX = 8;
let presets = null;              // null until the board's first snapshot
let presetSig = "";              // the JSON the buttons were drawn from
...
function estimateCost(keys) {
  const n = reqCount(keys);
  if (!n) return null;
...
function renderPresetCost() {
  // the price shares its row with all/none: both are pick-mode tools
  const show = paramMode === "edit" && !!catalog.length;
  $("#paramsTools").hidden = !show;
  if (!show) return;
  const c = estimateCost([...selectedKeys]);
  $("#presetCost").textContent = c
    ? `≈ ${c.hz.toFixed(1)} ${t("unit.hz")} · ${c.n} ${t("preset.reqs")}`
    : t("preset.costNone");
}

// A preset slot is edited by ticking, and every tick rolls the decoded CSV to a
// new file — a ride recorded through that is a pile of fragments. So opening a
// slot parks the logging intent and closing it puts it back.
let logBeforeEdit = null;

async function setEditLock(on) {
  if (on) {
    if (logBeforeEdit) return;
    const s = lastSnapshot || {};
    logBeforeEdit = { decoded: !!s.logging_decoded, raw: !!s.logging_raw };
    if (logBeforeEdit.decoded || logBeforeEdit.raw) {
      try {
        await api("/api/logging", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ decoded: false, raw: false }),
        });
        toast(t("preset.logPaused"), "warn");
      } catch (e) { toastErr(e); }
    }
  } else {
    const want = logBeforeEdit;
    logBeforeEdit = null;
    if (want && (want.decoded || want.raw)) {
      try {
        await api("/api/logging", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify(want),
        });
        toast(t("preset.logResumed"), "ok");
      } catch (e) { toastErr(e); }
    }
  }
  if (lastSnapshot) applySnapshot(lastSnapshot);   // repaint the two switches
}
...
function renderPresets() {
  ...
  presets.forEach((p, i) => {
    ...
    b.type = "button";
    b.className = "preset";
    const on = !!p.keys.length && sameSet(selectedKeys, p.keys);
    b.classList.toggle("is-on", on);
    b.classList.toggle("is-edit", editIdx === i);
    b.classList.toggle("is-was", lastIdx === i && !on);
    b.classList.toggle("is-empty", !p.keys.length);
    b.setAttribute("aria-pressed", on ? "true" : "false");
    // the count is the price of the preset: poll rate falls as the set grows
    b.textContent = `${presetName(p, i)} · ${p.keys.length}`;
    b.title = `${t("preset.aria")} ${presetName(p, i)}`;
    b.addEventListener("click", () => onPresetClick(i));
    box.appendChild(b);
  });
  const editing = paramMode === "edit" && editIdx >= 0;
  $("#presetEdit").hidden = $("#presetHint").hidden = !editing;
  if (editing) $("#presetName").style.gridColumn = String(editIdx + 1);
  // never while the rider is typing into it: the 0.2 s push would eat the letters
  if (editing && document.activeElement !== $("#presetName")) {
...
async function onPresetClick(i) {
  if (!presets) return;
  if (paramMode === "edit") {
    editIdx = editIdx === i ? -1 : i;      // tapping the open one closes it
    if (editIdx < 0) { await setEditLock(false); renderPresets(); return; }
    await setEditLock(true);               // no ride is recorded into a slot edit
    await applyPreset(i);                  // an empty slot is a legal starting point
    return;
  }
  if (!presets[i].keys.length) { toast(t("preset.empty"), "warn"); return; }
  await applyPreset(i);
}
...
async function applyPreset(i) {
  ...
  if (catalog.some((c) => pgroup(c) === "unknown" && want.has(c.key))) setUnknownOpen(true);
  lastIdx = i;
  const rolls = !!(lastSnapshot && lastSnapshot.log_decoded_file);
  await onSelectChange();
  // changing the column set closes the open CSV and starts a new one (_dec_restart)
  if (rolls) toast(t("preset.logRolled"), "ok");
}
...
$("#presetName").addEventListener("input", (e) => {
  if (editIdx < 0 || !presets) return;
  presets[editIdx].name = String(e.target.value || "").slice(0, PRESET_NAME_MAX);
  renderPresets();                      // the button relabels as it is typed
});
...
function applySnapshot(s) {
  ...
  if (scanT) {
    ...
    // scanning owns the port, and a slot edit would shred the CSV into fragments:
    // either one locks the log toggles
    const locked = !!s.scan_on || editIdx >= 0;
    dT.disabled = rT.disabled = locked;
    dT.closest(".rec-row").classList.toggle("is-locked", locked);
    rT.closest(".rec-row").classList.toggle("is-locked", locked);
    // say why the switch does not move instead of just greying it out
    if (locked) {
      const why = s.scan_on ? t("err.busy_scan") : t("err.busy_preset");
      $("#decMeta").textContent = $("#rawMeta").textContent = why;
    }
  }
...
function updateTestingButtons() {
  ...
  // test is locked out while another one runs
...
const boardLogs = makeLogBrowser({
  ...
  badge: (f) => (f.kind === "fw" ? { cls: "raw", text: "fw" }
    : f.kind === "update" ? { cls: "raw", text: "upd" } : { cls: "dec", text: "deb" }),
  view: (f) => (f.kind === "fw" ? "/api/firmware/log.txt?file="
    : f.kind === "update" ? "/api/update/log.txt?file=" : "/api/diag.txt?file=")
...
// CSV line splitter that respects double-quoted fields (exports often quote every
...
function parseCSV(text) {
  ...
  const isMs = (header[0] || "").toLowerCase().includes("ms");  // a "Time in ms" column
...
// ---------- software update ----------
// The board is not a repository and has no internet on the bike: an update is an
// archive the phone carries in. Everything that could go wrong is decided on the
// board (see app/web/update.py) — this only shows the verdict and the bar.
let updTimer = null;

function updEnter() { loadUpdate(); if (!updTimer) updTimer = setInterval(loadUpdate, 1500); }
function updLeave() { if (updTimer) { clearInterval(updTimer); updTimer = null; } }

async function loadUpdate() {
  let d;
  try { d = await api("/api/update"); } catch (e) { return; }
  const busy = d.op && d.op !== "idle";
  $("#updVersion").textContent = d.version || t("upd.unknownVersion");
  const res = d.result ? " · " + t("upd." + d.result) : "";
  const why = !busy && !d.result && d.busy ? " · " + t(d.busy) : "";
  $("#updState").textContent = t("upd." + (d.op || "idle")) + res + why;
  // the bar outlives the operation, exactly like the firmware one: the board
  // goes back to `idle` the moment the swap is done
  const p = d.prog || {};
  const pct = typeof p.percent === "number" ? p.percent : -1;
  const show = pct >= 0 && (busy || d.result);
  $("#updProg").hidden = !show;
  if (show) {
    $("#updFill").style.width = pct + "%";
    $("#updVal").textContent = p.total
      ? `${pct.toFixed(0)} % · ${p.done} / ${p.total}` : `${pct.toFixed(0)} %`;
    $("#updTrack").setAttribute("aria-valuenow", String(Math.round(pct)));
    $("#updTrack").setAttribute("aria-label", t("upd.progress"));
  }
  const log = $("#updLog");
  if (!log.hidden) {
    log.textContent = (d.log || []).join("\n");
  ...
  $("#updUploadLabel").classList.toggle("is-disabled", busy);
}

$("#updLogBtn").addEventListener("click", () => {
  const log = $("#updLog");
...
$("#updUpload").addEventListener("change", async (e) => {
  ...
  fd.append("file", file);
  try {
    await api("/api/update/upload", { method: "POST", body: fd });
    toast(t("upd.started"), "ok");
    updEnter();
  } catch (err) {
    // every failure the board reports is an i18n key, prose is the exception
    const key = String(err.message || "");
    toast(key.startsWith("err.") ? t(key) : t("banner.error") + " " + key, "err");
  }
  loadUpdate();
});
