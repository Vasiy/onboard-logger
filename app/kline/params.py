...
The ECU does NOT answer one big block: each live value is its own
...
RPM is ``15000000 / period``. So the map
...
class Param:
    ...
    # Which list the UI files this channel under. "known" = the meaning is confirmed on
    # the bike, "check" = named but still to be proven there,
    # "unknown" = no meaning at all. Empty falls back to ``default``, which is what the
    # grouping used to key off before the two ideas were split: ``default`` says what a
    # fresh device polls, ``group`` says where the row is drawn.
    group: str = ""
...
class ParamMap:
    ...
    def catalog(self) -> list[dict]:
        """Parameter metadata for the UI (key/name/unit/default/group/status map)."""
        return [
