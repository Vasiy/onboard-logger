"""Build a labelled ECU description from the ReadEcuIdentification data.
...
format (CRLF) saved next to a dump.
...
def _clean(chunk: bytes) -> str:
    # keep printable ASCII (spaces included — they are part of a field), drop binary
...
def describe(raw: bytes, hw: str = "", fields=DEFAULT_FIELDS, extra=None) -> str:
    """Render the ECU-id payload into the labelled text block (Date = today).
    ...
    and still ends in CRLF — the shape tests/test_firmware.py
