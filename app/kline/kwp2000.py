...
implemented:
  ...
  * 0x83 AccessTimingParameter      (test mode: timing params)
...
DIAG_SESSION = 0x81     # standard diagnostic session
...
# 0x00 is what the IAW 5AM family also answers
...
class KWP2000Session:
    ...
    def enter_test_mode(self) -> dict:
        ...
        This is the bring-up done right after StartCommunication:
        ``83 03 00 FF 00 FF 00``
    ...
    def read_ecu_identification_all(
        ...
        We used 0x80 alone; the IAW 5AM family also answers option 0x00.
    ...
    # status byte layout: low nibble = fault kind (1/2/4/8), bit 0x20 = stored
    # (vs current). Bit 0x20 is the one that decides — the coarser
    # "0x60 != 0 -> stored" also files a warning as stored. The wording behind the
    # four kinds is not published anywhere we can rely on, hence numbers.
    DTC_KINDS = (1, 2, 4, 8)
    ...
    # -- adaptation resets (verified for IAW 5AM) --
    def reset_tps(self) -> dict:
        ...
        # 0x33 confirms the routine on the families that answer it; the 5AM is
        # expected to reject this, which is fine and reported as-is.
    ...
    def routine_results(self, localid: int) -> dict:
        ...
        On the ECU families that support it, it is polled right after StartRoutine
        (``31 21`` then ``33 21``); the 5AM does not answer it, so this is a
        best-effort probe: a negative response means "this ECU has no
    ...
    def actuator_set(self, localid: int, on: bool) -> dict:
        ...
        There is no ECU-side timer: the output stays on until a 00 frame arrives,
        so the caller owns the timing (the worker does, via a deadline). Command
        byte 07/00, LocalID->actuator per the IAW test map.
    ...
    def read_local(
        ...
        ``with_addr=False`` uses the short header frame ``[len][data][cs]``, which
        some record ids are read with.
        """
